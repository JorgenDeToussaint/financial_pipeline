...
class AsyncManager:
                ...
                async with self.semaphore:
                    try:
                        ...
                        silver_path = PathManager.get_path(
                            "silver", display_id, now, "parquet", pipe.granularity
                        )
                        if self.s3.exists(bucket="silver", path=silver_path):
                            self.logger.info(
                                f"⏭️ [Task: {display_id}] Silver exists, skipping"
                            )
                            continue
                        ...
                        transformer = PipeFactory.get_transformer(
                            pipe.transformer_type, current_params
                        )
                        ...
                        bronze_path = PathManager.get_path(
                            "bronze", display_id, now, "json", pipe.granularity
                        )
                        ...
                        if silver_data is not None:
                            self.s3.save(
                                data=silver_data, bucket="silver", path=silver_path
                            )

                        self.logger.info(f"✅ [Task: {display_id}] Sukces")

                    except Exception as e:
                        self.logger.error(f"❌ [Task: {display_id}] Błąd: {e}")
    ...
    async def run_all(self):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [
                self._process_pipe(session, pipe, now) for pipe in self.config.pipes
            ]
            ...
            try:
                ...
                refinery.run(now)
                self.logger.info("✅ Gold layer made")
                view_builder = ViewBuilder(s3_client=self.s3)
                view_builder.build_all()
                self.logger.info("✅ Views ready")
