        ...
        for pipe in self.config.pipes:
            ...
            for item in items:
                ...
                if pipe.granularity == "hourly":
                    ...
                    path = f"/instrument={display_id}/{partition_path}/data_daily.parquet"
    ...
    def transform(self, conn, date: datetime):
        ...
        query = """
            ...
            with_returns AS (
                SELECT
                    *,
                    (price_usd / LAG(price_usd, 1) OVER (PARTITION BY ticker ORDER BY datetime)) - 1 AS return_1h,
                    (price_usd / LAG(price_usd, 24) OVER (PARTITION BY ticker ORDER BY datetime)) - 1 AS return_24h
                FROM joined_data
            )
            SELECT
                *,
